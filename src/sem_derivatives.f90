...
! nqdu remove negative sign, 2023.08.03
...
subroutine strain_monopole_td(u, G, GT, xi, eta, npol, nsamp, nodes, element_type, &
  ...
  strain_tensor(:,:,:,1) = grad_buff1(:,:,:,1) ! dsus
  strain_tensor(:,:,:,2) = f_over_s_td(u(:,:,:,1), G, GT, xi, eta, npol, nsamp, &
                                       nodes, element_type, logical(axial)) !dpup
  strain_tensor(:,:,:,3) = grad_buff2(:,:,:,2) !dzuz
  strain_tensor(:,:,:,4) = 0
  strain_tensor(:,:,:,5) = (grad_buff1(:,:,:,2) + grad_buff2(:,:,:,1)) / 2d0 !dsuz
  strain_tensor(:,:,:,6) = 0
...
!                                        + grad_buff2(:,:,:,2)) ! dzup 
  ...
  ! nqdu why there is a negative sign
  strain_tensor(:,:,:,4) = 0.5d0 * (f_over_s_td(u(:,:,:,3), G, GT, xi, eta, npol, &
                                                  nsamp, nodes, element_type, &
                                                  logical(axial)) &
                                       + grad_buff2(:,:,:,2)) ! dzup 
  strain_tensor(:,:,:,5) = (grad_buff1(:,:,:,2) + grad_buff3(:,:,:,1)) / 2d0 ! dzus
